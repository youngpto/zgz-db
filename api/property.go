...
// GetUserHeroProperty 获取玩家英雄所有属性值
func GetUserHeroProperty(userId int64, resourceHeroId int64) (*dto.UserHeroProperty, error) {
	result := new(dto.UserHeroProperty)
	// 先获取英雄表的基础属性和主键id
	heroInfo, err := service.FindHeroBaseProperty(resourceHeroId)
	if err != nil {
	...
	// 获取偏移值
	offset, err := service.FindPropertyOffsetByUserAndHero(userId, heroInfo.Id)
	if err != nil {
		return nil, err
	}
